#
# Train Gaussians with a baked environment cubemap as the far background,
# composited inside the rasterizer: the CUDA kernels blend Gaussians
# front-to-back and finish each pixel with `C += T_final * bg(pixel)` —
# equivalent to painting the cubemap first and alpha-blending back-to-front
# over it. Requires the patched diff-gaussian-rasterization (per-pixel
# background support in submodules/).
#
# The per-pixel background is the cubemap evaluated along each camera ray, so
# remaining transmittance is explained by the cubemap instead of forcing
# Gaussians to reconstruct the far background / sky.
#
# Based on train.py; cubemap-specific changes are marked with [CUBEMAP].
#
# Usage:
#   python train_cubemap_bg.py -s <source_path> -m output/<name> --eval
...
class CachedCubemapBackground:
    """[CUBEMAP] Per-camera background images, computed once and cached as uint8 on CPU."""

    def __init__(self, cubemap_dir):
        self.cubemap = CubemapBackground(cubemap_dir, device="cuda")
        self.cache = {}
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, cubemap_dir, loss_map_interval, loss_map_until):
    ...
    # [CUBEMAP] The rasterizer composites the cubemap per pixel, so the constant
    # bg color and random_background are not used.
    if not cubemap_dir:
        cubemap_dir = os.path.join(dataset.source_path, "cubemaps", "cubemap")
    print("Rasterizing over cubemap background: " + cubemap_dir)
    cubemap_bg = CachedCubemapBackground(cubemap_dir)
    if opt.random_background:
        print("[CUBEMAP] WARNING: --random_background is ignored, the cubemap is the background")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    # [CUBEMAP] Free-view preview; compute the background on the fly.
                    net_bg = cubemap_bg.cubemap.background_for_view(custom_cam)
                    net_image = render(custom_cam, gaussians, pipe, net_bg, scaling_modifier=scaling_modifer, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)["render"]
        ...
        with torch.no_grad():
            ...
            # Log and save
            # [CUBEMAP] Evaluate over the same cubemap background used for the training loss.
            composited_render = lambda viewpoint, pc: render(
                viewpoint, pc, pipe, cubemap_bg(viewpoint), use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, composited_render, (), dataset.train_test_exp)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    #gaussians.densify_and_prune(opt.densify_grad_threshold, 0.0005, scene.cameras_extent, None, radii)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Training script parameters (cubemap background composite)")
    ...
    # [CUBEMAP] "" default so get_combined_args-based tools keep working with this cfg_args.
